# See https://github.com/santec-corporation/TSL-Control-GUI/blob/feature-lan/main.py for other examples
...
class Operation:
    ...
    def connect_tsl(self):
        ...
        if self.tsl:
            try:
                ...
                idn_info = IDN.split(",")
                ui.initialize(idn_info)
                logging.info(f"Device Info - ProdName: {idn_info[1]}, SN: {idn_info[2]}, Firmware: {idn_info[3]}")
    ...
    def get_data():
        ...
        if Swp_mod == 1 or Swp_mod == 3:
            Arg1 = ui.ScanSpeed_input.currentText()
